...
- Delinquency codes 96 and 98 affect 269 unique rows. Their serious-delinquency rate is approximately 54.65%.
...
- 逾期字段中的 96 和 98 异常码共影响 269 行，这些记录的严重违约率约为 54.65%。
...
affect 269 rows and are strongly associated with the positive class, while
...
异常码影响 269 行，且与正类明显相关；额度使用率、负债比和收入存在明显长尾。
