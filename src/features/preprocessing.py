...
class CreditRiskPreprocessor:
    ...
    fitted_strategy_: str | None = field(default=None, init=False)

    def fit(self, train: pd.DataFrame) -> "CreditRiskPreprocessor":
        if self.strategy not in {"missing_plus_flag", "keep_raw"}:
        ...
        median_columns = ["MonthlyIncome", "NumberOfDependents", "age"]
        if self.strategy == "missing_plus_flag":
            median_columns.extend(self.delinquency_columns)
        self.medians_ = {}
        ...
        self.fitted_ = True
        self.fitted_strategy_ = self.strategy
        return self

    def transform(self, frame: pd.DataFrame) -> pd.DataFrame:
        if not self.fitted_:
            raise RuntimeError("CreditRiskPreprocessor must be fitted on Training before transform")
        if self.strategy != self.fitted_strategy_:
            raise RuntimeError("Preprocessing strategy cannot change after fit")
        ...
        if self.strategy == "missing_plus_flag":
            output["HasAbnormalDelinquencyCode"] = original.loc[
                :, list(self.delinquency_columns)
            ].isin(self.abnormal_values).any(axis=1).astype("int8")
        ...
        if self.strategy == "missing_plus_flag":
            schema.insert(-1, "HasAbnormalDelinquencyCode")
        output = output.loc[:, schema]
    ...
    def metadata(self) -> dict[str, object]:
        ...
        return {
            ...
            "abnormal_delinquency_strategy": self.fitted_strategy_,
    ...
    def output_schema(self) -> list[str]:
        ...
        if self.strategy == "missing_plus_flag":
            schema.append("HasAbnormalDelinquencyCode")
        schema.append("AgeInvalidFlag")
        return schema
